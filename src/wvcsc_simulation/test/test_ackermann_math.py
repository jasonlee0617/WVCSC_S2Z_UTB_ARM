...
def test_node_defaults_match_confirmed_real_vehicle_geometry():
    ...
    assert "declare_parameter('wheel_base', 0.82)" in source
...
def test_confirmed_real_geometry_has_expected_minimum_turning_radius():
    ...
    assert wheel_base / math.tan(max_steering_angle) == pytest.approx(1.5750728)


def test_steering_angle_command_uses_ackermann_kinematics():
    assert yaw_rate_from_steering(0.35, 0.20, 0.82) == pytest.approx(
        0.35 * math.tan(0.20) / 0.82)
...
def test_standard_twist_yaw_rate_is_limited_by_ackermann_curvature():
    expected = 0.35 * math.tan(0.48) / 0.82

    assert yaw_rate_from_twist(0.35, 1.0, 0.82, 0.48) == pytest.approx(expected)
    assert yaw_rate_from_twist(0.35, -1.0, 0.82, 0.48) == pytest.approx(-expected)
    assert yaw_rate_from_twist(-0.35, 0.10, 0.82, 0.48) == pytest.approx(0.10)
