...
def test_editor_round_trip_and_side_inference(tmp_path):
    editor = nav2_qt.MissionEditor()
    editor.start_pose = _pose(0.0, 0.0)
    editor.spray_duration = 2.5
    editor.return_home_after_finish = True
    editor.add_point(_pose(3.0, 0.5, 0.3))
    editor.add_point(_pose(5.0, -0.5, -0.2))
    ...
    assert [point.spray_side for point in loaded.points] == ['left', 'right']
    assert math.isclose(nav2_qt.pose_yaw(loaded.points[0].pose), 0.3)


def test_editor_loads_legacy_start_and_endpoint(tmp_path):
    ...
    editor = nav2_qt.MissionEditor()
    editor.load(path)
    assert editor.start_pose.position.x == 0.0
    assert len(editor.points) == 1
    assert editor.points[0].spray_side == 'right'
...
def test_single_uses_queued_target_and_removes_it_only_after_completion():
    editor = nav2_qt.MissionEditor()
    editor.start_pose = _pose(0.0, 0.0)
    editor.add_point(_pose(3.0, 0.5, 0.3), 'left')
    gui = _gui(editor)
    submitted = {}
    ...
    assert submitted['points'][0].spray_side == 'left'
