...
def validate_manual_request(
        request, *, map_frame, max_points, max_abs_coordinate,
        min_spray_duration, max_spray_duration, arm_base_forward_offset,
        arm_base_left_offset, arm_base_yaw):
    """Return validated route points and HOME pose from one request."""
    if request.header.frame_id != map_frame:
        raise ValueError(f'frame must be {map_frame}')
    if not request.mission_id.strip() or not request.points:
        raise ValueError('mission_id and points are required')
    if len(request.points) > int(max_points):
        raise ValueError(f'point count exceeds limit {max_points}')
    ...
    for item in request.points:
        ...
        if (not math.isfinite(duration) or
            ...
            raise ValueError(f'{point_id}: spray_duration out of range')
        ...
        if not all(math.isfinite(value) for value in (
            ...
            raise ValueError(f'{point_id}: non-finite arm-base tree offset')
        if math.hypot(tree_x_m, tree_y_m) < 1e-6:
            raise ValueError(f'{point_id}: arm-base tree offset is zero')
        if work_side != WorkSide.UNSPECIFIED:
            ...
            if abs(tree_y_m) <= 0.05 or work_side != expected_side:
                raise ValueError(
                    f'{point_id}: work_side conflicts with signed tree Y')
        ...
        if abs(tree_hint[0]) > bound or abs(tree_hint[1]) > bound:
            raise ValueError(f'{point_id}: derived tree hint out of bounds')
        points.append(RoutePoint(
            point_id, tree_hint[0], tree_hint[1], tree_hint[2], duration,
            docking, tree_x_m, tree_y_m, point_type, wide_spray_on_approach,
            dwell_time, work_side))
        seen.add(point_id)
    return points, home_pose
