...
class MoveIt2:
    ...
    def plan_async(
        ...
        if self.__joint_state is None:
            self._node._logger.debug(
                'Joint state not yet available; waiting for first message '
                'on /joint_states')
        while self.__joint_state is None:
            rclpy.spin_once(self._node, timeout_sec=0.05)
        start_joint_state = self.__joint_state
        self._node._logger.debug('Joint state available')
    ...
    def _send_goal_async_move_action(self):
        with self.__execution_mutex:
            ...
            future = self.__send_goal_future_move_action
    ...
    def __response_callback_move_action(self, response):
        with self.__execution_mutex:
            ...
            future = self.__get_result_future_move_action

        future.add_done_callback(self.__result_callback_move_action)
    ...
    def _send_goal_async_execute_trajectory(
        ...
        with self.__execution_mutex:
            ...
            future = self.__send_goal_future_execute_trajectory
    ...
    def __response_callback_execute_trajectory(self, response):
        with self.__execution_mutex:
            ...
            future = self.__get_result_future_execute_trajectory

        future.add_done_callback(self.__result_callback_execute_trajectory)
