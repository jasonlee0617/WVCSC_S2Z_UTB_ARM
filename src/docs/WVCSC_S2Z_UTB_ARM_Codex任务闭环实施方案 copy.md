...
Alicia-M 使用官方 `alicia_m_calibration` 的 20 个固定关节样本及原始顺序；仿真和实机共用同一个 C10 采集器。每个固定点仍逐个通过：

```text
目标关节的六轴完整性
→ URDF Jacobian 条件数 <14
→ 最小关节余量 ≥0.22 rad
→ MoveIt/OMPL RRTConnectFast 关节规划及碰撞检查
→ 执行并停稳
→ 连续 ArUco 图像质量门控
→ easy_handeye2 take_sample
```

固定表由基准正对码姿态、joint1 左右扰动、joint6 光轴旋转、joint5/joint4 倾斜、joint1+joint6 与多腕关节组合构成。动态 anchor、marker-relative 候选、bootstrap、临时外参和图像居中候选均已删除。任一点规划、停稳、图像质量或多样性失败时记录后继续下一点；达到环境的最低样本数立即停止，20点结束仍不足下限则失败且不导出。

### 5.2 采样、重心和求解门控

- 有效样本：实机最少 15，仿真最少 18；达到下限即结束固定表遍历；
- 求解子集：离群剔除后不得少于 14；
- 每姿态稳定 10 帧，停稳等待 1 s；
...
仿真标定场景与实机共享同一个 `auto_calibration_collector.py`、完整小车/车顶机械臂模型和车体碰撞几何。Gazebo 中的70 mm编码区域、90 mm背板 marker 相对 `alicia_base_link` 位于 `(0, 0.25, 0.002)`；启动文件仅用该值生成 marker，采集器仍以实际 marker TF 和C10图像质量采样。桌面与独立机械臂资产保留为注释参考，不参与默认启动。
...
2026-07-22 的自适应采样回归曾得到 22 个有效样本，`OpenCV/Park` 相对 URDF 真值
的误差为 **2.72 mm / 0.38°**；算法最大分歧 `0.20 mm / 0.19°`，固定标记残差
RMS `0.47 mm / 0.43°`。固定关节序列迁移后，必须重新完成同等真值回归；此历史
结果不得作为新的 20 点固定序列验收证据，也不替代实机重复标定、干式对准和湿喷落点验收。
